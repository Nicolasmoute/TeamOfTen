...
# Recurrence v2 (Docs/recurrence-specs.md) phase 8: the legacy
# in-memory `/repeat` clear-on-switch tests are gone. Recurrences are
# project-scoped DB rows now; the scheduler reads only the active
# project's rows so a switch automatically narrows what fires, and the
# previous in-memory clear logic was deleted along with the legacy
# `set_coach_repeat` / `get_coach_repeat` accessors.
