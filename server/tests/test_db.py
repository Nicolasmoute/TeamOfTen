...
async def test_init_db_creates_project_indexes(fresh_db: str) -> None:
    """Every project-scoped table must carry an idx_<name>_project
    index — the harness queries these on every pane open and event
    fan-out. Catches a regression where one falls out of SCHEMA."""
    await init_db()
    c = await configured_conn()
    try:
        cur = await c.execute(
