...
TICK_BASE_PROMPT = (
    ...
    "(2) Open coach-todos — for each open todo, find the smallest "
    "movable next step and take it. External blockers (pending user "
    "verdict, prod soak running, Player unavailable, harness env "
    "outside Coach scope) change WHAT the next move is, never "
    "WHETHER one exists. Zero-cost forward motions are always "
    "available: a probe DM to a silent Player, a status ping or "
    "clarifying question to the human, a read-only diagnostic, a "
    "fresh observation captured to memory, or splitting the todo "
    "into smaller pieces. Reject \"everything is gated\" and "
    "\"marginal value has gone to zero\" — those are rationalizations "
    "that mask a movable smallest-step. End this branch only after "
    "examining each open todo through this lens.\n"
