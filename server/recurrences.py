...
TICK_BASE_PROMPT = (
    ...
    "pending from the human or your teammates. Then scan "
    "\"## Active task health\" and \"## Stalled tasks\" if those "
    "sections appear in your system prompt above — auto-reassign "
    "fires at 2h, auto-archive at 4h. Intervene before the safety "
    "net does (nudge the blocker, reassign, or bump effort/model "
    "for repeat audit fails).\n"
    ...
    "finish it now and call coord_complete_todo. If a single turn "
    "isn't enough, the action you take must materially advance the "
    "SAME todo toward closure: assign it to a Player with a clear "
    "deliverable, draft and send the actual deliverable, write the "
    "decision, schedule the dependency. Probe DMs, status pings, "
    "and observations are legitimate ONLY when they directly enable "
    "the next closure step on the same todo — never as standalone "
    "\"I touched something\" activity. Skimming across todos to drop "
    "a micro-touch on each is the antipattern this branch exists to "
    "prevent. External blockers (pending user verdict, prod soak, "
    "Player unavailable, env outside scope) change WHAT the next "
    "closure step is, never WHETHER one exists. Reject \"everything "
    "is gated\" and \"marginal value has gone to zero\" — they are "
    "rationalizations. Pick the most closable item, focus on it, "
    "close it (or move it visibly closer to closed), then stop.\n"
