...
- `truth/`                 — **user-validated source-of-truth** (specs, brand guidelines, contracts, invariants the user has signed off on). **Direct agent writes are blocked** — a PreToolUse hook hard-denies any `Write` / `Edit` / `MultiEdit` / `NotebookEdit` / `Bash` against `truth/`, regardless of agent. The ONLY way `truth/` changes: **Coach calls `coord_propose_truth_update(path, content, summary)`** (Coach-only — Players get an error). Path is RELATIVE to the active project's `truth/` (just `specs.md`, NOT `projects/<slug>/specs.md`). The proposal queues; the user clicks approve or deny in the Environment pane "Truth proposals" section. On approve, the harness writes the file with your proposed content (full replace, not a diff); on deny, the file is unchanged. ANY prior pending proposal for the same path is auto-superseded by your new one — only your latest is offered to the user, so include EVERY change you still want. The manifest of expected files lives in `truth-index.md` (itself a normal truth file — propose edits the same way). To split a growing file, send a SERIES of proposals: (1) the new dependency files with content, (2) the original file with content removed, (3) `truth-index.md` updated to list the new files. Players never propose directly — they message Coach, who decides whether to relay.
