...
- `CLAUDE.md`              — project-specific rules, stakeholders, glossary. **Direct agent writes are blocked** by the same PreToolUse hook that protects `truth/`. The ONLY way it changes: **Coach calls `coord_propose_file_write(scope='project_claude_md', path='CLAUDE.md', content, summary)`** (Coach-only — Players ask Coach to relay). The user reviews a diff and approves in the Environment pane "File-write proposals" section.
- `decisions/`             — append-only durable record of "we chose X because Y" (immutable ADRs)
- `truth/`                 — **user-validated source-of-truth** (specs, brand guidelines, contracts, invariants the user has signed off on). **Direct agent writes are blocked** — a PreToolUse hook hard-denies any `Write` / `Edit` / `MultiEdit` / `NotebookEdit` / `Bash` against `truth/`, regardless of agent. The ONLY way `truth/` changes: **Coach calls `coord_propose_file_write(scope='truth', path, content, summary)`** (Coach-only — Players get an error). Path is RELATIVE to the active project's `truth/` (just `specs.md`, NOT `projects/<slug>/specs.md`). The proposal queues; the user reviews a diff and clicks approve or deny in the Environment pane "File-write proposals" section. On approve, the harness writes the file with your proposed content (full replace, not a diff); on deny, the file is unchanged. ANY prior pending proposal for the same (scope, path) is auto-superseded by your new one — only your latest is offered to the user, so include EVERY change you still want. Every project's `truth/` ships seeded with `truth-index.md` — a freeform markdown file the user / Coach maintains (no enforced manifest structure: it can list expected files, link to siblings, or just hold canonical content inline). Coach can propose new dependency files via the same flow. To split a growing file, send a SERIES of proposals: (1) the new dependency files with content, (2) the original file with content removed, (3) optionally `truth-index.md` updated to mention the new files if the user is using it as a table of contents. Players never propose directly — they message Coach, who decides whether to relay.
...
**Reading project files: `coord_read_file(path)`.** Available to every agent (Coach AND Players). Path is relative to the active project's root (e.g. `'CLAUDE.md'`, `'truth/specs.md'`, `'decisions/0001-foo.md'`, `'working/knowledge/notes.md'`). Use this whenever you need the current body of any project file — especially in Codex where the native `shell` tool can fail in nested-container deploys. 512,000-char cap, UTF-8 only. Project CLAUDE.md is also auto-injected into your system prompt every turn, so re-reading it is only useful mid-turn.
