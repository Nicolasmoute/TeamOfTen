...
> disagree, truth-index wins. This file goes deeper on Compass (lattice,
...
The original spec put the burden on Coach to "decide when to call audit". The first auto-audit pass (early 2026-05) fired on every artifact event the team produced — `commit_pushed`, `decision_written`, `knowledge_written`, `output_saved` — but most of those are intermediate execution work, and the kanban lifecycle (canonical spec at [Docs/kanban-specs-v2.md](kanban-specs-v2.md); v1 archive at [Docs/kanban-specs-v1-archived.md](kanban-specs-v1-archived.md)) already runs syntactic + semantic Player audits and shipper review at every stage transition. Compass's downstream auditing was duplicative noise.

The refocused watcher ([server/compass/audit_watcher.py](../server/compass/audit_watcher.py)) sits one layer **upstream**: it subscribes to a single event family — `task_stage_changed` — and audits **only the `from='plan' to='execute'` transition**. That's the moment a planner has finished writing `spec.md` and the executor is about to start. Compass checks that the **plan aligns with intent** (the lattice's directional claims); kanban's own auditor / shipper stages handle whether execution aligns with the plan. **Under v2 (shape-(2) routing) every Compass verdict — including `aligned` — also writes a `compass_audit` row in the per-project event log so Coach sees WHY the lattice signed off, not just THAT it did.** See [kanban-specs-v2.md §19](kanban-specs-v2.md).
...
`AUDIT_SYSTEM` ([server/compass/prompts.py](../server/compass/prompts.py)) is shaped specifically for plan artifacts. The verdict is about whether the plan's *direction* matches the lattice's intent, not about prose style or implementation details. Verdict bands are unchanged from §5.2 (`aligned` / `confident_drift` / `uncertain_drift`); what's intent-aware is the framing — the LLM treats settled lattice rows as VALIDATED DIRECTION the plan should respect.
  ...
  - **Model choice**: Compass calls run on the latest Sonnet at `medium` effort. The value is the **alias** `latest_sonnet` resolved at call time via `models_catalog.resolve_model_alias`. Hardcoded — there is no env var and no UI knob, so the team-wide policy ("Compass runs on Sonnet at medium effort") is uniform across every deploy. To bump the tier when Anthropic ships a newer Sonnet, update only `_ALIAS_TO_CONCRETE` in [server/models_catalog.py](../server/models_catalog.py).
  - **Codex fallback**: when the primary Claude path fails, Compass routes the same prompt through a one-shot Codex call ([server/compass/codex_llm.py](../server/compass/codex_llm.py)). Two trigger conditions: (a) `_call_claude` raises `CompassLLMError` (subprocess died, auth gone, rate-limited / 5h Max block, network outage); (b) `_call_claude` returns `CompassLLMResult(is_error=True)` (CLI surfaced a soft error result). Fallback model is `latest_mini` at `medium` effort (Codex tier alias resolved through the same `models_catalog`). Also hardcoded — same rationale as the primary. **Per-run latching**: inside `runner.run()` (which wraps the pipeline in `llm.begin_run_latch_scope()` / `end_run_latch_scope()`), the first failure flips a contextvar and every subsequent stage in the run skips Claude entirely — saves the wasted retry cost during a multi-stage pipeline. Standalone calls (the auto-audit watcher) inherit `latched=False` and retry on Codex per call. Codex calls use a fresh `codex app-server` subprocess per call, an ephemeral thread (no persistence in `agent_sessions`), no MCP, and `read-only` sandbox. Cost: `plan_included` (ChatGPT auth → $0.0) or priced via `codex_cost_usd` (api_key auth); ledger row written under `agent_id="compass"`, `runtime="codex"`, with the same `cost_basis` label as the equivalent Claude call so the rollup is unified. If both runtimes fail, the original Claude error (raise or `is_error=True` result) is what the caller sees.
