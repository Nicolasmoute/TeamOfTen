# Kanban task lifecycle — Specification (v1, ARCHIVED)

> ## HISTORICAL ARCHIVE — superseded by [kanban-specs-v2.md](kanban-specs-v2.md)
>
> This document describes the v1 (auto-routing) kanban that was deployed
> until 2026-05-07. It is preserved verbatim for historical reference.
> It is **NOT authoritative** for current behavior. The canonical kanban
> spec is [kanban-specs-v2.md](kanban-specs-v2.md).
>
> Do not extend this document. Do not link to it as a source of truth.
> Cross-references that point here are historical "Recent" entries
> describing what v1 was at the time those entries were written —
> they intentionally point at the archive, not at v2.
>
> Archived 2026-05-07.
...
**Status:** Shipped (2026-05-07, v0.3.14 — `coord_complete_execution` artifact gate: when the executor passes `artifact_path` it's now resolved under the project root and verified to exist on disk before the role row closes; when no path is passed, the response carries an imperative self-check reminder ("did you actually produce a file? did you save it?") so executors stop archiving tasks whose deliverable was never written).
**Target:** TeamOfTen multi-agent harness (Python, Claude Agent SDK, kDrive-backed shared state, single-VPS)
**Version:** 0.3.14

> **v0.3.14 (2026-05-07 coord_complete_execution artifact gate + reminder)** — production trace 2026-05-07: a Player called `coord_complete_execution(task_id, summary)` to close a diagnostic task. The spec required a report at a specific path on disk. The Player closed without `artifact_path`, the kanban took the close at face value, the role row archived, and no file was ever written. Coach found the gap on a follow-up sweep — the kanban accepted "I'm done" without verifying the deliverable existed.
>   - **Path verification when present.** `coord_complete_execution(..., artifact_path=...)` ([server/tools.py:2842](../server/tools.py)) now resolves the path under `project_paths(active).root`. Relative paths are joined to the project root; absolute paths are accepted iff they `relative_to()` the project root (so a typo like `/etc/passwd` can't be probed via this tool). If the file doesn't exist, the call is rejected BEFORE any DB write — the role row stays open so the executor can save the file and retry without a Coach intervention. The response on success now appends "(verified on disk)" so the executor sees the gate ran.
>   - **Self-check reminder when absent.** When no `artifact_path` is passed, the response includes an imperative checkpoint: "CHECKPOINT before your turn ends: did you actually produce an artifact? If yes, did you save it to disk? Run a quick listing on the path the spec named — if the file isn't there, write it NOW via Write / coord_save_output / coord_write_knowledge / coord_write_decision BEFORE this turn ends. ... If the work was genuinely path-less ... pass completion_kind='noop' next time so the absence is logged intentionally." Doesn't structurally enforce — pairs with the v0.3.11 imperative-response pattern (agents act on framed checklists). Catches the bug surface where the structural gate can't help (executor passed no path).
>   - **Containment + resolution edge cases.** Path-resolution failures (broken symlinks, unreadable parents) bounce as 'could not resolve' errors rather than crashing. Outside-project paths bounce with a pointer at the conventional lanes (`working/knowledge/reports/...`, `outputs/...`, `decisions/...`). Tool description grew an "ARTIFACT GATE (v0.3.14)" paragraph naming the rule + the pre-write requirement.
>   - **Tool description tightened.** "don't pre-declare a path you haven't written yet — save the file first ... then call this." Closes the misuse pattern where Players were treating `artifact_path` as an *intent* declaration instead of a *recorded fact*.
>   - **Tests:** 1207 → 1212 (+5 in [test_kanban_tools.py](../server/tests/test_kanban_tools.py)): existing happy-path test extended to write the file first + assert the "verified on disk" suffix; +4 new (rejects-missing-artifact, rejects-path-outside-project, no-path-includes-checkpoint, absolute-path-inside-project-ok). New helpers `_write_misc_file` and `_seed_executor_role` extracted for reuse.
>   - **Out of scope (intentionally):** parsing spec.md frontmatter for a structured `deliverable_path` field and verifying *that specific* path regardless of what the Player passed (rigorous-but-spec-changing fix C from the design discussion). Defer until path-mismatch (Player wrote a file but at the wrong location) actually bites — current gate catches the more common "didn't write anything" case.
...
>   - **Gate.** When the target stage isn't `archive`, the active `task_role_assignments` row for the target stage's role must end up with at least one eligible Player. Either Coach passed `assignee` (we plant a fresh row via `_assign_role_helper`) or the existing active row already has `owner` or non-empty `eligible_owners`. Without one, the call is rejected with a pointer at the right `coord_assign_<role>(task_id=..., to=<slot>)` template (audit roles include `kind=` / `focus=`).
>   - **Atomic plant.** The role row is inserted *before* `tasks.status` is updated, so `_assign_role_helper`'s wake-check (which reads current `tasks.status`) returns False — no double-wake. Then `tasks.status` updates, `task_stage_changed` publishes, and the subscriber's `_on_stage_changed` finds the planted row and fires the single stage-entry wake. Trajectory mirror happens too — the assignment lands in `tasks.trajectory.to[stage]` so the board UI + Coach prompt + future transitions stay in sync.
>   - **Archive escape.** `stage='archive'` skips the gate entirely (no role to fill) and explicitly rejects `assignee` (programmer error, not silent drop).
>   - **Tests:** 1189 → 1194 (+5 in `test_kanban_tools.py`: rejects-when-no-assignee, plants-role-row-atomically, pool-assignee-accepted, archive-rejects-assignee, existing-assignee-no-assignee-needed). Existing `test_advance_task_stage_stamps_last_stage_change_at` and `test_stage_progression_resets_escalation_level` updated to pass `assignee` since their seeded trajectories had empty role rows.

> **v0.3.11 (2026-05-06 response-strengthening pass)** — production trace 2026-05-06: a Player got a planner assignment, called `coord_my_assignments`, saw "Pending planner: t-...", reported it back, and STOPPED — treated the descriptive response as a status report. v0.3.10 added a `## Next action:` footer to that one tool, but the audit (see [Docs/kanban-messaging-inventory.md](kanban-messaging-inventory.md) + [Docs/kanban-followup-proposals.md](kanban-followup-proposals.md)) found 15 more places with weak/missing follow-ups. Every kanban tool response and Coach-bound event payload now ends with an imperative call to action — named completion tool with `task_id` baked in, "what to expect next" framing, no descriptive-only status reports.
>   - **Tool responses (12)** — `coord_create_task` (auto-wake reminder + warning when first stage has no eligible owner), `coord_assign_task`/`coord_assign_planner`/`coord_assign_auditor`/`coord_assign_shipper` (hard-assign vs pool vs future-stage reservation, each with the right "do NOT follow up; the wake covers it" framing), `coord_write_task_spec` (planner role complete + auto-advance), `coord_submit_audit_report` (branched: pass → "auto-advances, you're done"; fail → "executor re-woken with your report"), `coord_commit_push` (linked-task vs auto-bound vs unbound vs push-failed branches), `coord_complete_execution` (executor role complete + audit caveat), `coord_mark_shipped` (auto-archives + Coach summarizes), `coord_update_task` (manual archive → "no auto-summary; you decide what to tell the user"), `coord_advance_task_stage` (same archive framing), `coord_set_task_trajectory` (stand-down + role-call wakes), `coord_set_task_blocked` (sweeper-ignores / ladder-restarts).
>   - **Event payload bodies (3)** — `stage_assignment_needed` gains a `body` with the role-specific `coord_assign_<role>(...)` template baked in. `commit_without_task_id_warning` names the recovery (link via `coord_advance_task_stage` or accept it's scratch). `audit_fail_notification` branches on `escalate`: first fail → "expected correction noise, watch round 2"; ≥ 2 same-kind fails → "ESCALATION: bump effort/model" with the executor slot named in the call template.
>   - **Helper added** — `_role_token_for_stage(stage)` in `tools.py` maps a stage to its assign-tool token (planner/task/auditor/shipper) for response strings that point Coach at the right follow-up tool. `audit_syntax`/`audit_semantics` collapse to `auditor` because `coord_assign_auditor` takes the kind as a param.
>   - **Tests:** 1168 → 1189 (+21 in `test_response_followups.py` covering every new branch). Pre-existing assertions on lowercase "wrote spec" / "completed execution" widened to case-insensitive (the responses now use sentence case).
>   - **Out of scope (intentionally):** `coord_set_task_workflow` (low-stakes), non-kanban tools (memory/knowledge/decisions/messaging/players/runtime/Compass), wake bodies that already carry strong follow-ups (`_wake_role_or_emit_needed`, `send_role_stand_down`, stall-ladder rungs, `_emit_task_completed`).

> **v0.3.9.1 (2026-05-05 audit pass on v0.3.9)** — three issues caught on the v0.3.9 self-audit:
>   - **MEDIUM — wake body misled about Telegram routing.** The original prompt told Coach "if this turn was user-triggered, the bridge will forward your text to the user's phone automatically." But `task_completed` IS the trigger event, and it's a system event — the Telegram outbound filter (v0.3.5) only forwards Coach turns whose triggering event was `message_sent{from='human', to='coach'}`. So Coach reading the misleading prompt would type a plain reply, expect Telegram delivery, and the user on Telegram would never see it. Wake body now names the explicit channels: `coord_send_message(to='broadcast', body=...)` for harness-UI delivery, `coord_request_human(subject, body, urgency='normal')` for unconditional Telegram delivery, and explicitly states "this wake is system-triggered, so the Telegram bridge does NOT auto-forward."
>   - **LOW — `_transition` re-emitted phantom events on no-op transitions.** A bogus call where `old_status == new_status` (replay, double-emit) would still UPDATE timestamps and emit `task_stage_changed{from: X, to: X}` + (for archive) a duplicate `task_completed`, double-waking Coach. Defensive short-circuit added at the top of `_transition` — bail silently when `old_status == new_status`. Caller upstream guards (e.g. `_on_task_shipped`'s `if t["status"] != "ship": return`) already prevent the canonical case; this is the backstop.
>   - **LOW — publish vs wake exception coupling.** `bus.publish` was outside the wake's try/except, so a publish failure would skip the wake too — Coach would get neither the event nor the prompt. Each is now independently wrapped: a publish failure still lets the wake fire (Coach hears about the completion via the prompt) and a wake failure doesn't undo the published event.
>   - **Tests:** 1165 → 1168 (+3 in `test_task_completion_notify.py`: explicit-channel-tools-not-misleading-Telegram, no-op transition is silent, publish failure does not cancel wake).
>   - **Other audit notes (no fix needed):** `last_stage_owner` query doesn't filter `superseded_by IS NULL` (in normal flow ORDER BY assigned_at DESC picks the active row anyway, so cosmetic only). `executor` shows `tasks.owner` post-rung-3-reassign (correct: the current owner is the one who finished the work). Title quoting via `f"{title!r}"` may render with single or double quotes depending on the title's contents — purely cosmetic.
...
| `coord_advance_task_stage` | `task_id, stage, assignee?, note?` | Explicit override. Bypasses the trajectory gate. `assignee` is required when the target stage's role row has no eligible owner; pass single slot or comma-list pool. Archive ignores `assignee`. |
...
| `coord_complete_execution` | `task_id, summary, artifact_path?, completion_kind?` | Non-git execution completion for research, writing, marketing, ops, or no-diff artifacts. Marks executor row complete and emits `task_execution_completed`. **v0.3.14 artifact gate:** when `artifact_path` is present, the harness resolves it under `project_paths(active).root` (relative paths joined; absolute paths must contain-check inside the project root) and rejects the call if the file doesn't exist on disk — so executors can't pre-declare a path they haven't written. The role row stays open on rejection so the executor can save the file and retry. When `artifact_path` is absent, the response carries an imperative "CHECKPOINT before your turn ends — did you actually produce a file? did you save it?" reminder + a pointer at `completion_kind='noop'` for genuinely path-less work, so executors don't archive tasks whose deliverable was never written. |
