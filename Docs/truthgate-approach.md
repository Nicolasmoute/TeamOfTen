...
- `coord_run_truthgate`: Coach-only tool that runs the classifier for a task in `truthgate`, persists verdict/basis/concerns/method/model fields, emits `task_truthgate_started`, `task_truthgate_completed`, and `task_truthgate_blocked` when the verdict requires amendment or clarification. Existing verdicts are preserved unless Coach passes `force=true`; classifier failures fail closed by blocking the task without recording a pass/override verdict. Action-needed verdicts and classifier failures also send Coach an interrupt inbox message/wake naming the task, verdict/failure, reason, basis/concerns, and expected action. It does not advance the stage or wake a Player.
- `coord_refresh_truthgate_basis`: Coach-only audit-stage repair path for administrative stale-basis blocks. It applies only to tasks already in `audit_syntax` or `audit_semantics` with a pass/override verdict and a cited truth file modified after `truthgate_at`. It stamps a new `truthgate_at` plus Coach rationale, emits `task_truthgate_basis_refreshed`, and does not record audit PASS, rerun the classifier, advance the stage, or wake a Player.
...
| `truthgate_needs_human_clarification` / `truthgate_rejected_or_needs_human_clarification` | Task is too vague, contradicts truth in a way warranting discussion, or shouldn't be done as stated. | Coach escalates via `coord_request_human`, rewrites/archives the task, records an allowed override, or deliberately leaves it blocked. |
...
The implemented targeted truth check is a focused guard, not a corpus audit or full TruthScore run. It reads only the task's cited `truth_basis` files for auditor wake context. Missing, unreadable, or malformed cited basis metadata is surfaced for Coach review; audit PASS is rejected when the report identifies a cited-truth violation or when the cited basis cannot be checked. If a cited truth file changed after the recorded TruthGate run, PASS is rejected as administrative staleness, Coach gets an interrupt action-needed signal, and the auditor is told not to submit a false FAIL solely for staleness. Coach can inspect the truth change and call `coord_refresh_truthgate_basis(task_id, rationale)` to refresh the checkpoint, or reroute/rework/archive deliberately. Empty-basis sparse/override tasks skip the file read with a visible warning.
