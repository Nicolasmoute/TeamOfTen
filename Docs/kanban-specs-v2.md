...
| `coord_propose_truth_amendment` | `task_id, path, content, summary, rationale, evidence?, affected_docs?, provisional_impl?, rejection_consequence?, draft_model?, drafted?` | Coach-only wrapper over the protected file-write proposal flow. Queues a normal `file_write_proposals` row with `scope='truth'`, `metadata_json`, and `originating_task_id`; sets `tasks.truthgate_pending_proposal_id` for correlation; and leaves human approval/denial unchanged. It never writes `truth/` directly and does not create a separate amendment registry. |
