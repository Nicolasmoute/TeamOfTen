...
- `trajectory[0].to` may be a single name, a list (pool), or empty. It remains FYI only: top-level task creation lands in Backlog, backlog promotion inserts the task into `truthgate` with no Player role/wake, and Coach picks the first real assignee when explicitly approving `truthgate → plan|execute`.
- Subsequent entries' `to` follow the same advisory shape. Coach picks each later stage's assignee at `coord_approve_stage` time, which requires a single named slot for every non-archive transition.
...
- **Players** execute, review, ship, and verify. Coach can also delegate planning to a Player — after Backlog promotion and a TruthGate pass/override, Coach assigns the planner role via `coord_approve_stage(stage='plan', assignee=<slot>)`.
...
Carried from v1 §4.6 with TruthGate context added: `auditor_syntax` is contract-bound (does the deliverable match what was asked + is it internally sound?); `auditor_semantics` is context-bound (does this make sense in the world this project lives in — Compass + truth/ + wiki/, NOT spec). Semantic audits require a stated `focus`. Syntax audits use a contract cascade (spec → title/description → executor wake → commit) so a missing spec doesn't stall the auditor.

When a task has TruthGate fields, auditor wakes include a `## TruthGate context` block with the recorded verdict/method, `truth_basis`, `truth_concerns`, sparse warnings, provisional status, and closure reference. The targeted helper reads only cited `truth_basis` files; it is not a project-wide `/truthscore` run. Missing/stale/unreadable cited files are surfaced as warnings for Coach review rather than crashing the wake. Empty-basis sparse/override tasks skip the targeted file read and show the skip warning.

`coord_submit_audit_report(verdict='pass')` is rejected when the targeted TruthGate guard reports that the audit body itself identified a cited-truth violation, or when the cited basis cannot be checked. The auditor should submit `verdict='fail'` and cite the violated/unverifiable truth clause, or message Coach for rerouting. FAIL submissions remain the normal path for reporting targeted truth violations. Stale cited-basis metadata is different: if a truth file changed after the recorded TruthGate run, PASS is rejected fail-closed, but the tool tells the auditor not to submit a false FAIL solely for administrative staleness and sends Coach an interrupt action-needed message. Coach may inspect the truth change and call `coord_refresh_truthgate_basis(task_id, rationale)` to refresh the audit checkpoint, or reroute/rework/archive deliberately. The refresh does not record PASS, rerun the classifier, advance the stage, or wake unrelated Players.
...
| Tool | Params | Purpose |
|---|---|---|
| `coord_create_task` | `title, description?, parent_id?, priority?, workflow?, tracking_reason?, trajectory?, success_criteria?` | Creates a top-level Backlog item or a child task. For top-level tasks (`parent_id` omitted), records the title/description/trajectory as a pending Backlog entry and does **not** create an active task row, plant a role, change a Player tool allowlist, or wake a Player. `trajectory[0].to` may be a single name, pool, or empty list because it is advisory until Coach dispatches work. When Coach promotes the Backlog item, the active task enters `truthgate` with the trajectory preserved as its post-gate plan and automatically runs the TruthGate assessment; no Player role is planted/woken while in `truthgate`. After a TruthGate pass/override, Coach explicitly calls `coord_approve_stage(task_id, next_stage='plan'|'execute', assignee=<slot>, note=...)` to plant the first role and wake that Player. Subsequent trajectory entries also remain FYI only until Coach approves into that stage. **`success_criteria`** is the optional Coach-authored "definition of done" — see §17.3. |
...
| `coord_run_truthgate` | `task_id, force?` | Coach-only manual rerun/repair path. Normal Backlog promotion automatically starts and awaits the TruthGate classifier. This tool reruns the classifier for an existing task in `truthgate`, persists verdict/basis/concerns/method/model fields, emits `task_truthgate_started` and `task_truthgate_completed`, and emits `task_truthgate_blocked` plus sets the task blocked when the verdict requires truth amendment or human clarification. Existing verdicts are preserved unless `force=true`; classifier failures fail closed by blocking the task without recording a pass/override verdict. Action-needed verdicts and classifier failures send Coach an interrupt inbox message/wake naming the task, verdict/failure, reason, basis/concerns, and expected action. Does **not** move the task or wake a Player; Coach still calls `coord_approve_stage` after a pass. |
| `coord_refresh_truthgate_basis` | `task_id, rationale` | Coach-only audit-stage stale-basis repair path. Applies only while the task is in `audit_syntax` or `audit_semantics`, already has a TruthGate pass/override verdict, and the targeted guard detects a cited `truth_basis` file modified after `truthgate_at`. Updates `truthgate_at` and records Coach's rationale in `truthgate_warning`, emits `task_truthgate_basis_refreshed`, and leaves the audit role/stage untouched. Does **not** record audit PASS, rerun the classifier, advance the stage, or wake unrelated Players. |
| `coord_record_truthgate_override` | `task_id, kind, rationale, closure_reference?` | Coach-only. Records `truthgate_coach_override` or `truthgate_emergency_override` with required non-empty rationale, emits `task_truthgate_override_recorded` and `task_truthgate_completed`, and marks emergency overrides provisional. Emergency overrides may store an optional `closure_reference` for later reconciliation. Does **not** move the task or wake a Player. |
| `coord_propose_truth_amendment` | `task_id, path, content, summary, rationale, evidence?, affected_docs?, provisional_impl?, rejection_consequence?, draft_model?, drafted?` | Coach and active Player roles. Wrapper over the protected file-write proposal flow. Queues a normal `file_write_proposals` row with `scope='truth'`, `metadata_json`, and `originating_task_id`; sets `tasks.truthgate_pending_proposal_id` for correlation; and leaves human approval/denial unchanged. It never writes `truth/` directly and does not create a separate amendment registry. `draft_instruction` / LLM amendment drafting is deferred. |
| `coord_record_provisional_closure` | `task_id, closure_reference` | Coach-only. Records the reconciliation reference required before a provisional emergency-override task can be delivered to archive, emits `task_provisional_closure_recorded`, and does **not** move the task or wake a Player. Accepted forms are `amendment:<proposal_id>` (proposal exists and targets `truth/`; delivered archive requires approval), `none_needed:<rationale>`, or `rollback:<task_id>` (referenced task exists). |
| `coord_archive_task` (NEW — R3) | `task_id, summary` | Coach-only deliberate archive. Writes the user-facing summary, transitions to archive, marks any active role rows complete. The summary lands as a `.sys` row in Coach's pane and is forwarded to Telegram if the originating turn was user-triggered. Use this when a task wraps via natural completion (work delivered) or explicit cancellation (Coach decides not to ship); v1's auto-archive on trajectory-end is gone. Delivered archive rejects provisional tasks until a valid `closure_reference` is recorded; `amendment:<proposal_id>` closures must point at an approved `truth/` proposal. |
...
| `coord_submit_audit_report` | `task_id, kind, body, verdict, on_behalf_of?, message_to_coach?` | Runs the targeted TruthGate guard for PASS submissions, then writes `audits/audit_<round>_<kind>.md` with frontmatter; records the verdict on the auditor role row; marks the role row complete. `on_behalf_of` carries over. Emits `audit_report_submitted{task_id, kind, verdict, report_path, message_to_coach, on_behalf_of?, ...}`. **Verdict='fail' does NOT auto-revert** (R2) — surfaces to Coach via event log; Coach decides. |
| `coord_run_verifier_smoke` | `task_id, target, smoke, params?` | Verifier-only server-side smoke proxy available only while the caller has an active `verify` role row for that task. Runs named allowlisted read-only checks such as `health_detail`, `status_summary`, `task_board_read`, `agent_context`, and `turns_summary`; rejects arbitrary URLs, methods, headers, bodies, tokens, cookies, and unknown params. Returns sanitized `PASS`/`FAIL`/`SKIPPED`/`BLOCKED` evidence suitable for `coord_submit_verification_report`. Remote targets without server-side verifier auth config fail closed with explicit `BLOCKED` limitations; raw `HARNESS_TOKEN`, bearer material, cookies, session ids, and secret plaintext are never returned. |
...
| POST | `/api/tasks` | Human task composer. Body `{title, description?, parent_id?, priority?, workflow?, tracking_reason?, trajectory?, emergency?, emergency_rationale?}`. Without `parent_id`, non-emergency requests create a pending Backlog item; promotion later creates the active task in `truthgate` and automatically runs the TruthGate assessment. Top-level `emergency=true` requires `emergency_rationale`, records a promoted Backlog row, creates the active task in `truthgate` with no Player role/wake, and automatically runs the TruthGate assessment before returning. With `parent_id`, creates a child task under the parent using the existing direct-dispatch path. The composer may omit `trajectory` for the top-level Backlog default `[{"stage":"execute","to":[]}]`; child tasks require an explicit single-owner first stage. |
...
| POST | `/api/tasks/{id}/approve_stage` (NEW — backstop for `coord_approve_stage`) | Body `{next_stage, assignee, note?}`. Human-side equivalent for manual interventions. `next_stage` accepts `verify`; no UI verify actions are required for this backend-only rollout beyond API compatibility. Like `coord_archive_task`, delivered `next_stage='archive'` rejects provisional tasks without valid closure. |
| POST | `/api/tasks/{id}/cancel` | Human cancellation. Sets `cancelled_at` so the archive view distinguishes cancellation from delivery; cancellation remains available for provisional tasks because it is recorded as cancellation, not delivered closure. |
...
- **TruthGate audit guard.** Audit wakes carry targeted TruthGate context from the task's cited `truth_basis`. If an auditor reports a cited-truth violation, they submit FAIL; PASS is rejected when the report itself identifies a TruthGate violation or the cited basis cannot be checked.
...
- Do your role's work, then signal Coach with the right completion tool:
  ...
  - shipper (GitHub-backed repo) → `coord_ship_to_dev(task_id)` — exposed in shipper role allowlists, enforces the audit-pass gate, consumes the executor `commit_pushed` event for the task, cherry-picks or resumes the executor commit on a temp branch off `origin/dev`, opens/reuses a GitHub PR, squash-merges it, closes the shipper role row, and wakes Coach. If cherry-pick conflicts are resolved manually with `git cherry-pick --continue`, rerun `coord_ship_to_dev` to resume the existing temp branch; unresolved conflicts stay fail-closed. Already-present patches on `origin/dev` complete through explicit ship evidence without a PR, including whether verification came from commit ancestry or `git cherry` patch-id equivalence. Empty cherry-picks that cannot be verified already present fail closed and require formal ship evidence or Coach reroute. **Recommended tool for all ship-stage work.** Raw `git push origin ...:dev` bypasses the audit gate and is a pb-005 violation.
  ...
  - verifier → optionally run `coord_run_verifier_smoke(task_id, target, smoke, params?)` for server-side allowlisted protected smokes, then `coord_submit_verification_report(task_id, verdict, body, message_to_coach?, evidence?)` — for optional post-ship verification. Smoke evidence is sanitized and verifier-only; FAIL is reported to Coach only and does not auto-revert or create follow-up work.
