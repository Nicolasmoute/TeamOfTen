...
| `coord_run_truthgate` | `task_id, force?` | Coach-only. Runs the TruthGate classifier for a task in `truthgate`, persists verdict/basis/concerns/method/model fields, emits `task_truthgate_started` and `task_truthgate_completed`, and emits `task_truthgate_blocked` plus sets the task blocked when the verdict requires truth amendment or human clarification. Does **not** move the task or wake a Player; Coach still calls `coord_approve_stage` after a pass. |
| `coord_record_truthgate_override` | `task_id, kind, rationale` | Coach-only. Records `truthgate_coach_override` or `truthgate_emergency_override` with required non-empty rationale, emits `task_truthgate_override_recorded` and `task_truthgate_completed`, and marks emergency overrides provisional. Does **not** move the task or wake a Player. |
