...
- **Reach the team from anywhere.** The web UI is the primary workspace, and it has a **mobile-optimized layout** for phones — bottom rail, swipeable single-pane deck, full-screen env overlay — so you can keep tabs on the team from a phone without pinching at a desktop layout. A **Telegram bridge** lets you send goals to Coach and read replies from your phone when you're away from a screen entirely; whitelist-gated, escalations (`coord_request_human`) ping you too. Everything human-readable also mirrors to your cloud drive, so the agents' output is reachable even with the harness offline.
